...
var AIHubProjectName = '${abbrs.AIHubProject}${resourceToken}'
...
// Deploy CoreAI Infrastructure

module coreAI 'core/ai/CoreAI Deploy.bicep' = if (switches.CoreDeploy) {
  scope: rg
  name: 'coreAI'
  params: {
    location: location
...
module hub 'core/ai/hub.bicep' = if (switches.CoreDeploy) {
  scope: rg
  name: 'hub'
  params: {
    name: AIHubName
    ...
    storageAccountId: coreAI.outputs.storageAccountId
    keyVaultId: coreAI.outputs.keyVaultId
    openAiName: coreAI.outputs.openAiName
    aiSearchName: coreAI.outputs.searchServiceName
    openAiConnectionName: '${coreAI.outputs.openAiName}-connection'
    aiSearchConnectionName: '${coreAI.outputs.searchServiceName}-connection'
  }
}
...
module webapps 'app/webapps.bicep' = if (switches.AppDeploy) {
  ...
  params: {
    ...
    openAiProps: {
      endpoint: coreAI.outputs.openAiEndpoint
...
output storageAccountName string = (switches.CoreDeploy) ? coreAI.outputs.storageAccountName : ''
output searchServiceName string = (switches.CoreDeploy) ? coreAI.outputs.searchServiceName : ''
output openAIEndpoint string = (switches.CoreDeploy) ? coreAI.outputs.openAiEndpoint : ''
